...
import { Generation, PokemonWeaknesses } from 'src/app/models/PokemonWeaknesses';
...
export class HomePage {
  ...
  constructor(private api: PokemonsService, private router: Router, private modalController: ModalController) {
    ...
    this.api.getPokemons().pipe(
      switchMap(data => {
        ...
        return forkJoin(data.results.map(pokemon => {
          return this.api.getDetailPokemon(pokemon.url);
        }));
      })
    ).subscribe(data => {
  ...
  buscarPokemon(event: Event) {
    ...
    if (value && value.trim() !== '') {
      // Filtrar la lista de Pokémon en función del valor de búsqueda
      this.listaPokemon = this.listaPokemon.filter((pokemon: Pokemon) => {
        return pokemon.name.toLowerCase().startsWith(value.toLowerCase());
      });
    } else {
      // Si el campo de búsqueda está vacío, obtener la lista completa de Pokémon
      this.getListaPokemon();
    }
  }
